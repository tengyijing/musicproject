				...
				if (data[0] == 3) {
					if (data[3][i].parentmid == 12) {
						if (data[3][i].mid == data[0]) {
							str += "<li><a href='#' class='header-nav__cur'>"
									+ data[3][i].mname + "</a></li>";
						} else {
							str += "<li><a href='/" + data[3][i].ename
									+ "?menuid=" + data[3][i].mid + "' >"
									+ data[3][i].mname + "</a></li>";
						}
					...
					if (data[3][i].parentmid == 12) {
						...
						if (data[3][i].mid == data[1]) {
							str1 += "<li><a href='#' class='subNav-cur'>"
									+ data[3][i].mname + "</a></li>";
						} else {
							str1 += "<li><a href='/" + data[3][i].ename
									+ "?menuid=" + data[3][i].mid + "' >"
									+ data[3][i].mname + "</a></li>";
						}
					} else {
...
function videoSearch() {
	$.ajax({
		...
		success : function(re) {
			window.location.href = "SearchMusicServlet.do?musicName=" + re
					+ "&page=1";
		},
				...
				success : function(data) {
					for (var i = 0; i < data.length; i++) {
						...
						if(mid==17){
							...
							str += "<a  title='"+ data[i].title+ "' style='text-decoration:none;' href='play?id="+data[i].id+"' target='_new'>"+data[i].title+"</a><span class='artist'>";
							str += " <span class='author_list'  title='"+ data[i].sname+ "' style='float:right' ><a hidefocus='true' href='play?id="+data[i].id+"' target='_new'>"+ data[i].sname+ "</a>";
