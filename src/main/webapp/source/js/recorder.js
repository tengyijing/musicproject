(function(exports){
    ...
    var Recorder = function(config){
        ...
        config.sampleRate = config.sampleRate || 8000; //采样频率，默认为44100Hz(标准wav采样率)
        config.bitRate = config.bitRate || 1411; //比特率，默认为128kbps(标准wav质量)
                    ...
                    switch(e.data.cmd){
                        ...
                        case 'end':
                            if(successCallback){
                                ...
                                Util.log('wav大小：' + blob.size + '%cB', 'color:#0000EE');
                            }
                            break;
                        case 'error':
