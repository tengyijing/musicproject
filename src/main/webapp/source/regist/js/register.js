...
document.write('<script src="source/regist/pictureYanZheng/js/jquery.idcode.js"><\/script>');
...
function CheckUserName(){
	var User;
	$.ajax({
	    url : "checkUserName.do", //(默认: 当前页地址) 发送请求的地址
...
function emailBinding(){
	var User;
	$.ajax({
	    url : "emailBinding.do", //(默认: 当前页地址) 发送请求的地址
		type: "post", //(默认: "get") 请求方式 ("post" 或 "get")， 默认为 "get"。注意：其它 http请求方法，如 put和 delete也可以使用，但仅部分浏览器支持。
		async:false,//(默认: true) 默认设置下，所有请求均为异步请求。如果需要发送同步请求，请将此选项设置为 false。注意，同步请求将锁住浏览器，用户其它操作必须等待请求完成才可以执行。
		contentType:"application/x-www-form-urlencoded",//(默认: "application/x-www-form-urlencoded") 发送信息至服务器时内容编码类型。默认值适合大多数应用场合。
...
function phoneBinding(){
	var User;
	$.ajax({
	    url : "phoneBinding.do", //(默认: 当前页地址) 发送请求的地址
		type: "post", //(默认: "get") 请求方式 ("post" 或 "get")， 默认为 "get"。注意：其它 http请求方法，如 put和 delete也可以使用，但仅部分浏览器支持。
		async:false,//(默认: true) 默认设置下，所有请求均为异步请求。如果需要发送同步请求，请将此选项设置为 false。注意，同步请求将锁住浏览器，用户其它操作必须等待请求完成才可以执行。
		contentType:"application/x-www-form-urlencoded",//(默认: "application/x-www-form-urlencoded") 发送信息至服务器时内容编码类型。默认值适合大多数应用场合。
		data: {tel:$("#phone").val(),
...
function sentPhone(){
	$.ajax({
	    url : "sentPhone.do", //(默认: 当前页地址) 发送请求的地址
		type: "post", //(默认: "get") 请求方式 ("post" 或 "get")， 默认为 "get"。注意：其它 http请求方法，如 put和 delete也可以使用，但仅部分浏览器支持。
		async:false,//(默认: true) 默认设置下，所有请求均为异步请求。如果需要发送同步请求，请将此选项设置为 false。注意，同步请求将锁住浏览器，用户其它操作必须等待请求完成才可以执行。
		contentType:"application/x-www-form-urlencoded",//(默认: "application/x-www-form-urlencoded") 发送信息至服务器时内容编码类型。默认值适合大多数应用场合。
		data: {tel:$("#phone").val(),//userName:$("#adminNo").val(),
		},
		dataType:'json',
         success: function(json, textStatus){//如果调用servlet成功，响应200。请求成功后回调函数。这个方法有两个参数：服务器返回数据，返回状态(可以缺省)。
			console.log(textStatus);
			var flag = json.flag;
			if (flag == 'true') {
				$("#successEmail").html("验证码已发送至您的手机，请输入短信中的验证码，确保您的手机号码真实有效。");
			} else if (flag == 'false') {
				$("#successEmail").html("发送失败，请重新发送，确保您的手机号码真实有效。");
			}
		},
...
function yanzhengma(){
	var reEmail = false;
	$.ajax({
	    url : "yanZhengPhone.do", //(默认: 当前页地址) 发送请求的地址
		type: "post", //(默认: "get") 请求方式 ("post" 或 "get")， 默认为 "get"。注意：其它 http请求方法，如 put和 delete也可以使用，但仅部分浏览器支持。
		async:false,//(默认: true) 默认设置下，所有请求均为异步请求。如果需要发送同步请求，请将此选项设置为 false。注意，同步请求将锁住浏览器，用户其它操作必须等待请求完成才可以执行。
		contentType:"application/x-www-form-urlencoded",//(默认: "application/x-www-form-urlencoded") 发送信息至服务器时内容编码类型。默认值适合大多数应用场合。
		data: {yanzheng:$("#verifyNo").val(),
},
		//发送到服务器的数据。将自动转换为请求字符串格式。GET 请求中将附加在 URL 后。查看 processData 选项说明以禁止此自动转换。必须为 Key/Value 格式。如果为数组，jQuery 将自动为不同值对应同一个名称。如 {foo:["bar1", "bar2"]} 转换为 '&foo=bar1&foo=bar2'。
		dataType:'json',/*预期服务器返回的数据类型。如果不指定，jQuery 将自动根据 HTTP 包 MIME 信息返回 responseXML 或 responseText，并作为回调函数参数传递，可用值: 
						 *"xml": 返回 XML 文档，可用 jQuery 处理。 
