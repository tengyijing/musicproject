...
<script type="text/javascript" src="source/singer/js/modernizr.js"></script>
 ...
 <!-- 分页样式 -->
<link rel="stylesheet" type="text/css" href="source/SearchMusic/fenye/css/zxf_page.css" />
  ...
  <header class="header">
    <div class="header-container"  style="width:1200px;">
      ...
      <ul class="header-subNav" style="margin-top:20px;">
        ...
        <li><a href="getDynamic.do" class="subNav-cur">音乐圈</a></li>
  ...
  <div class="pb-container"> 
   <div class="pb-container-main pb-after-clear"> 
    ...
    <div class="header-login3">
          <a  href="getDynamic.do" class="open-vip" id="quanbu" ">全部动态</a>
          <a  href="getPartDynamic.do" class="open-green" id="haoyou"">好友动态</a>
    </div>
     ...
     <div class="commmain" style="margin-top:30px;"> 
       ...
       <div class="con f-r" style="margin-right:50px;"> 
        <div id="nameicon" class="center_header4" style="margin-top:-10px;float:left;">
         <c:choose>
        <c:when test="${dynamic.imgstr== null or dynamic.imgstr==''}">
            <a href="getFriendSong.do?user=${dynamic.dynamicer}"><img src="source/images/user.png" id="img1"/></a>
        </c:when>
        <c:otherwise>
        <a href="getFriendSong.do?user=${dynamic.dynamicer}"><img src="${dynamic.imgstr}" id="img2"/></a>
        </c:otherwise>
        </c:choose>
         </div>
        <div style="margin-left:50px;margin-top:13px;font-size:17px;"><a href="getFriendSong.do?user=${dynamic.dynamicer}" style="display:inline">${dynamic.dynamicer }</a></div>
        <p class="f-l">${dynamic.dynamicContent }</p> 
        <p><a style="display:inline">详情</a></p>
        <div class="date-dz">
                    <span class="date-dz-left pull-left comment-time">${dynamic.dynamicDate }</span>
                    <div class="date-dz-right pull-right comment-pl-block">
                       <c:choose>
                       <c:when test="${dynamic.dynamicer==username}">
                       <a href="javascript:;" class="removeBlock" del="${dynamic.dynamicId}">删除</a>
                       </c:when>
                       </c:choose>
                        
                        <a  class="date-dz-pl pl-hf hf-con-block pull-left" hp="0" onclick="hideTalk(this)">回复</a>
                        <span class="pull-left date-dz-line">|</span>
                        <a href="javascript:;" class="date-dz-z pull-left"><i class="date-dz-z-click-red"></i>赞 (<i class="z-num"  good="${dynamic.dynamicId}">${dynamic.good}</i>)</a>
    ...
    <div class="reviewArea clearfix">
        <textarea  class="content comment-input" placeholder="说点什么吧" onkeyup="keyUP(this)" hp2="${dynamic.dynamicId}"></textarea>
        <a class="plBtn" style="color:#fff">评论</a>
    </div>
    <!--评论区域 end-->
    <!--回复区域 begin-->
    <div class="comment-show">
    <c:forEach items="${dynamic.myReview}" var="myReview" varStatus="index2">
    <div class="comment-show-con clearfix">
    <div class="comment-show-con-img pull-left">
    <div id="nameicon" class="center_header4" style="margin-top:-10px;float:left;">
    <c:choose>
    <c:when test="${myReview.imgstr== null or myReview.imgstr==''}">
         <a href=""><img src="source/images/user.png" id="img1"/></a>
    </c:when>
    <c:otherwise>
    <a href=""><img src="${myReview.imgstr}" id="img2"/></a>
     ...
     <div class="pl-text clearfix">
      <a href="getFriendSong.do?user=${myReview.reviewer}" class="comment-size-name" p3="${myReview.reviewId}" p4="${dynamic.dynamicId}">${myReview.reviewer}</a>
       <span class="my-pl-con">&nbsp;${myReview.reviewContent}</span>
        </div>
         <div class="date-dz"> 
         <span class="date-dz-left pull-left comment-time">${myReview.reviewDate}</span>
          <div class="date-dz-right pull-right comment-pl-block">
          <a href="javascript:;" class="date-dz-pl pl-hf hf-con-block pull-left" style="margin-right:40px;">回复</a>
           </div>
            </div>
            <c:forEach items="${myReview.myRevert}" var="myRevert" varStatus="index3">
            <div class="all-pl-con" style="background-color:#EAEAEC">
            <div class="pl-text hfpl-text clearfix">
            <a href="#" class="comment-size-name">${myRevert.reverter}</a>
            <span class="my-pl-con">回复<span style="color:red">@<a style="color:#fa3b4a;display:inline;font-size:13px;" href="getFriendSong.do?user=${myRevert.toReverter}">${myRevert.toReverter}</a></span>&nbsp;${myRevert.revertContent}</span>
            </div>
            <div class="date-dz">
             <span class="date-dz-left pull-left comment-time">${myRevert.revertDate}</span>
              <div class="date-dz-right pull-right comment-pl-block">
               <a href="javascript:;" class="date-dz-pl pl-hf hf-con-block pull-left" style="margin-right:32px;">回复</a>
                </div>
                 </div>
                 </div>
            </c:forEach>
    ...
    <!--回复区域 end-->
     ...
     <div class="commside"> 
      ...
      <div class="pb-iblock pb-fr pb-after-clear pb-main-side"> 
       <a href="myMusic.do"><img class="pb-block" src="source/images/musicCircle.png" style="width:295px"/></a> 
       <h6 class="pb-mt15 pb-clr1 pb-size-big">	当前擂主</h6> 
       <div class="pb-line2 pb-mt5"> 
       </div>
       
       <c:forEach items="${registList}" var="registList" varStatus="index4">
       <div id="nameicon" class="center_header5" style="margin-bottom:10px;float:left;">
      <c:choose>
        <c:when test="${registList.imgstr== null or registList.imgstr==''}">
            <a href="getFriendSong.do?user=${registList.username}"><img src="source/images/user.png" id="img1" /></a>
        </c:when>
        <c:otherwise>
        <a href="getFriendSong.do?user=${registList.username}"><img src="${registList.imgstr}" id="img2"/></a>
        </c:otherwise>
        </c:choose>
        </div>
       <a href="getFriendSong.do?user=${registList.username}" class="tle pb-mt5" style="margin-right:20px;padding-top:20px;font-size:18px;">&nbsp;&nbsp;&nbsp;&nbsp;${registList.username}</a>
       
        
       <div class="pb-line2 pb-mt5"> 
       </div> 
       <a href="getFriendSong.do?user=${registList.username}" class="but pb-clr1 pb-size-normal pb-mt15" style="background-color: #E3494B;">添加关注</a>
       </c:forEach> 
      </div> 
      <!--右侧我要投稿公共部分--> 
      ...
      <div class="sidewrap  pb-mt40"> 
       ...
       <div style="height:480px;overflow:hidden;" class="con pr"> 
        <div style="left:150px;height:150px;top:15px;" class="single"> 
         <a title="清纯唯美写真合集" href=""><img alt="唯美写真" src="source/images/qing_10001_7b053ee001_310.jpg" style="width:145px;" /></a> 
        </div> 
        <div style="left:0px;height:150px;top:15px;" class="single"> 
         <a title="也许我就是一块老墨" href=""><img alt="也许我就是一块老墨" src="source/images/qing_10001_2278c48f96_310.jpg" style="width:145px;" /></a> 
        </div> 
        <div style="left:150px;height:150px;top:170px;" class="single"> 
         <a title="证件照" href=""><img alt="" src="source/images/qing_10001_3c506bc67d.jpg" style="width:145px;" /></a> 
        </div> 
        <div style="left:0px;height:150px;top:170px;" class="single"> 
         <a title="" href=""><img alt="" src="source/images/essay_10001_0ba5781188.jpg" style="width:145px;" /></a> 
        </div> 
        <div style="left:150px;height:150px;top:325px;" class="single"> 
         <a title="爱情就是一百多年的孤寂" href=""><img alt="爱情就是一百多年的孤寂" src="source/images/qing_10001_5502d62ca8_310.jpg" style="width:145px;" /></a> 
        </div> 
        <div style="left:0px;height:150px;top:325px;" class="single"> 
         <a title="深林幽静" href=""><img alt="深林幽静" src="source/images/qing_9999_578544d87f_310.jpg" style="width:145px;" /></a> 
        </div> 
       </div> 
      </div> 
     </div> 
		...
		addhtml:function(obj,pageinit){
			return (function(){
				...
				if (pageinit.current > 1) {
					obj.append('<a href="getDynamic.do?page=${pageNo-1}&musicName=${musicName}" class="prebtn">上一页</a>');
				} else{
					obj.remove('.prevPage');
				...
				for (;start <= end;start++) {
					if ((start <= pageinit.pageNum) && (start >=1)) {
						if (start == pageinit.current) {
							...
							obj.append('<a href="getDynamic.do?page='+start+'&musicName=${musicName}" class="zxfPagenum nextpage">'+ start +'</a>');
						}else{
							obj.append('<a href="getDynamic.do?page='+start+'&musicName=${musicName}" class="zxfPagenum">'+ start +'</a>');
						}
					}
				}
				...
				if (pageinit.current >= pageinit.pageNum) {
					...
					obj.append('<a href="getDynamic.do?page=${pageNo+1}&musicName=${musicName}" class="nextbtn">下一页</a>');
				}
				/*尾部*/
				obj.append('<span>'+'共'+'<b>'+pageinit.pageNum+'</b>'+'页，'+'</span>');
				obj.append('<span><form action="getDynamic.do" method="post" id="from1">到第<input type="text" value="${musicName}" name="musicName" style="display:none;"><input type="text" name="page" class="zxfinput"/>页<button type="submit" class="zxfokbtn" style="margin-top:5px; margin-left:5px;">确定</button></form></span>');
